...
export class CollectionPermissions extends PureComponent {
	...
	render() {
		const { viewstate, handlers } = this.props
		...
		const { catalog, department, section } = this.props.viewstate.course

		const { username } = this.props.viewstate.user
		...
		const sort = (data, sortType) => {
			if (this.state.sortType.reverse === false) {
				...
				data.sort((a, b) => {
					...
					case `Name`:
						return b[`account-name`].localeCompare(
							a[`account-name`],
							{ sensitivity: `base` }
						)
					default:
				...
				data.sort((a, b) => {
					...
					case `Name`:
						return a[`account-name`].localeCompare(
							b[`account-name`],
							{ sensitivity: `base` }
						)
					default:
					...
					<CourseTable id='course-table'>
						...
						<form onSubmit={handlers.addCourse}>
							<DepartmentSelect
								...
								value={department}
								onChange={handlers.handleDepartmentChange}
							...
							<CatalogInput
								...
								min='0'
								onChange={handlers.handleCatalogChange}
								onKeyPress={(e) => {
									if (e.charCode === 13)
										handlers.handleCatalogBlur(e)
								}}
								onBlur={handlers.handleCatalogBlur}
								value={catalog}
							...
							<SectionInput
								...
								min='0'
								onChange={handlers.handleSectionChange}
								onKeyPress={(e) => {
									if (e.charCode === 13)
										handlers.handleSectionBlur(e)
								}}
								onBlur={handlers.handleSectionBlur}
								value={section}
					...
					<UserListTable>
						<UserList id='user-table'>
							...
							<Search
								...
								onSubmit={handlers.addTA}
							>
								<input
									className='faculty-input'
									type='search'
									placeholder={`Enter netID`}
									onChange={handlers.handleUserTAChange}
						...
						<UserList id='user-table'>
							<TableHeader>
								...
								<div>
									...
									<AddManyButton
										type='button'
										onClick={handlers.AddBatchNetids}
									>
										Add many...
									</AddManyButton>
								</div>
