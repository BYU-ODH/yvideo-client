			...
			<InnerContainer>
				<Column>
					{/* only shows only for the admin */}
					{
						...
						<h4>
						Public
							<SwitchToggle on={collection.public} setToggle={makePublic} data_key='public' />
						</h4>
					}
				</Column>
			</InnerContainer>
			<TableContainer>
				...
				<CourseTable id='course-table'>
					...
					<form onSubmit={addCourse}>
						<DepartmentSelect className='department-select' value={department} onChange={handleDepartmentChange} placeholder='Dept - Ex: ENGL'/>
						<CatalogInput className='catalog-input' min='0' onChange={handleCatalogChange} onKeyPress={e => {
							if (e.charCode === 13) handleCatalogBlur(e)
						}} onBlur={handleCatalogBlur} value={catalog} placeholder='Course - Ex: 101' required/>
						<SectionInput className='section-input' min='0' onChange={handleSectionChange} onKeyPress={e => {
							if (e.charCode === 13) handleSectionBlur(e)
						}} onBlur={handleSectionBlur} value={section} placeholder='Section - Ex: 001' required/>
							...
							{ courses.length > 0 ?
								courses.map((element, index) =>
									<tr key={index}>
										...
										<td onClick={e => removeCourse(element[`id`])}><img src={removeIcon} alt='' width='20px'/></td>
									</tr>,
								)
								:
								null
				...
				<UserListTable>
					<UserList id='user-table'>
						...
						<Search className='faculty-submit' onSubmit={addTA}>
							<input className='faculty-input' type='search' placeholder={`Enter netID`} onChange={handleUserTAChange} value={userTA.username} />
								...
								{userTA.length > 0 ?
									userTA.map((element, index) =>
										<tr key={index}>
											...
											<td onClick={e => removeUser(element[`username`])}><img src={removeIcon} alt='' width='20px'/></td>
										</tr>,
									)
									: <></>
								}
						...
						<TableHeader>
							...
							<div>
								<Search className='faculty-submit' onSubmit={addUser}>
									<input className='faculty-input' type='search' placeholder={`Enter netID`} onChange={handleUserChange} value={username} />
									<AddButton className='add-faculty-button' type='submit' disabled={disabledUser}>Add</AddButton>
								</Search>
								<AddManyButton type='button' onClick={addBatchNetids}>Add many...</AddManyButton>
							</div>
						</TableHeader>
						<Table border='1'>
							<thead>
