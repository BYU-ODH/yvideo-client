				...
				<div className='menu-modal' onMouseLeave={e => setShowSpeed(false)}>
					...
					<div>
						{ playbackOptions.map((playbackAtIndex) => (
							playbackAtIndex != 1 ?
							<><input type='button' value={playbackAtIndex} onClick={() => handlePlaybackRateChange(playbackAtIndex)} className={playbackRate == playbackAtIndex ? `active-value` : ``}/><br/></>
							:
							<><input type='button' value='Normal' onClick={() => handlePlaybackRateChange(playbackAtIndex)} className={playbackRate == playbackAtIndex ? `active-value` : ``}/><br/></>
							))
						}
					</div>
				</div>
