...
const TrackEditorSideMenu = props => {
	...
	const handleEditComment = (value, cEvent, int) => {
		// document.getElementById(`save-comment`).disabled = false
		switch (int) {
		case 1:
								...
								<tbody className={`censor-list`}>
									{event.type === `Censor` &&
										Object.keys(event.position).sort((a, b) => parseFloat(event.position[a][0]) - parseFloat(event.position[b][0])).map((item, i) => (
											<tr className={`${activeCensorPosition === item && `censor-active`}`} key={item} >
												<td id='time-td'><input
													id={`censor-time-input-${item}`}
													className='censor-row'
													type='text'
